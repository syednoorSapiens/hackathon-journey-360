...
import { callSmartSchemaAPI } from "./apiCall";
// Mock AI Parser - Returns Travel Insurance or Death Claim Journey based on input
export class AIParser {
  static async parseUserStory(input: string): Promise<FormSchema> {
    const defaultSchema = this.getTravelInsuranceSchema();
    try {
      const smartSchema = await callSmartSchemaAPI(input);
      console.log("Received schema from Smart Schema API:", smartSchema);
      return smartSchema;
      // return smartSchema ? JSON.parse(smartSchema) : defaultSchema;
    } catch (error) {
      console.error("Error parsing user story with Smart Schema API:", error);

...
import { isEmbedMode } from "./utils/embedDetection";
...
export default function App() {
  ...
  // Check if we're in iframe mode and skip to editor
  useEffect(() => {
    const checkEmbedMode = async () => {
      // Add delay to ensure DOM is ready
      setTimeout(() => {
        const embedded = isEmbedMode();
        console.log('🔍 Checking embed mode:', embedded);
        console.log('📍 Window.self:', window.self);
        console.log('📍 Window.top:', window.top);
        console.log('📍 Are they equal?', window.self === window.top);
        setIsEmbedded(embedded);
        
        if (embedded && state.screen === "login") {
          console.log('✅ Detected iframe mode - Auto-loading editor');
          // Auto-login and load demo schema when in iframe
          setState((prev) => ({
            ...prev,
            screen: "editor",
            inputMode: "text",
            requirements: "Travel insurance form with trip details, traveler information, coverage selection, and payment",
            isProcessing: true,
            isLoggedIn: true,
          }));
          
          // Load schema asynchronously
          AIParser.parseUserStory(
            "Travel insurance form with trip details, traveler information, coverage selection, and payment"
          ).then((demoSchema) => {
            const tests = TestGenerator.generateTests(demoSchema);
            const mockApi = MockApiGenerator.generateEndpoints(demoSchema);
...
`;
  ...
  return (
    <div className='h-screen overflow-hidden bg-background flex flex-col'>
      <Toaster />
      {/* Top Navigation - Hide when in iframe mode */}
      {state.screen !== "login" && !isEmbedded && (
        <TopNav
          darkMode={darkMode}
